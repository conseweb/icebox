...
func generateSpend(flagPrivateKeys string, flagDestination string, flagRedeemScript string, flagInputTx string, flagAmount int) string {
	...
	scriptPubKey, err := btcutil.NewP2PKHScriptPubKey(publicKeyHash)
	if err != nil {
		log.Fatal(err)
	}
	//Create unsigned raw transaction
	//scriptSig in unsigned transaction is serialized redeemScript of input P2SH transaction.
	rawTransaction, err := btcutil.NewRawTransaction(flagInputTx, flagAmount, redeemScript, scriptPubKey)
	if err != nil {
		log.Fatal(err)
	}
...
func signMultisigTransaction(rawTransaction []byte, orderedPrivateKeys [][]byte, scriptPubKey []byte, redeemScript []byte, inputTx string, amount int) ([]byte, error) {
	...
	for i, privateKey := range orderedPrivateKeys {
		signatures[i], err = btcutil.NewSignature(rawTransaction, privateKey)
		if err != nil {
			return nil, err
		}
	}
	...
	if len(redeemScript) < 255 {
		requiredOP_PUSHDATA = btcutil.OP_PUSHDATA1 //OP_PUSHDATA1 specifies next *one byte* will be length to be pushed to stack
		redeemScriptLengthBytes = []byte{byte(len(redeemScript))}
	} else {
		requiredOP_PUSHDATA = btcutil.OP_PUSHDATA2 //OP_PUSHDATA2 specifies next *two bytes* will be length to be pushed to stack
		redeemScriptLengthBytes = make([]byte, 2)
		binary.LittleEndian.PutUint16(redeemScriptLengthBytes, uint16(len(redeemScript)))
	}
	//Create scriptSig
	var buffer bytes.Buffer
	buffer.WriteByte(byte(btcutil.OP_0)) //OP_0 for Multisig off-by-one error
	...
	signedRawTransaction, err := btcutil.NewRawTransaction(inputTx, amount, scriptSig, scriptPubKey)
	if err != nil {
		return nil, err
	}
